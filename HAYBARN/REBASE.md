...
Assume the current Haybarn branch is based on `v1.5.5` and we are moving to
`vX.Y.Z`.
...
git rebase --onto vX.Y.Z v1.5.5 haybarn
```
...
1. Update the embedded version references that mention the previous base (`1.5.5`):
