...
The `haybarn` core fork must carry the upstream `v1.5.5` tag — the extension
build (`extension-ci-tools`) runs `git checkout v1.5.5` inside the checked-out
caller repo. Push it explicitly (a branch push does not carry tags):

```sh
git push origin refs/tags/v1.5.5
```
...
```sh
git tag haybarn-v1.5.5-rc1
git push origin haybarn-v1.5.5-rc1
```
