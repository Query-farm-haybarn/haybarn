...
string ShellState::GetDefaultDuckDBRC() {
	duckdb::LocalFileSystem lfs;
	return lfs.JoinPath(GetHomeDirectory(), ".haybarnrc");
}
...
int RunShell(int argc, const char **argv) {
	...
	** is given on the command line, look for a file named ~/.haybarnrc and
	** try to process it.
	*/
	...
	if (!data.readStdin) {
		...
		*/
		...
		if (data.stdin_is_interactive) {
			...
			// 'haybarn-v*'` (e.g. "1.5.5-rc1" for a tag-at-HEAD release,
			// "1.5.5-rc1-3-g5f26f0d" for commits past it, or "dev-g5f26f0d"
