...
static void ShowUnsupportedStorageVersionError(const idx_t version_number) {
	// Haybarn preserves DuckDB's upstream storage-format versions for compatibility.
	...
	if (!version.empty()) {
		...
		version_text = "a newer version of Haybarn / DuckDB";
	} else {
		version_text = "an older development version of Haybarn / DuckDB";
	}
	throw IOException(
