...
> [!IMPORTANT]
> **Haybarn 1.5.2 is in release-candidate phase.** The current tag is
> `haybarn-v1.5.2-rc13`. Final `1.5.2` will arrive once the extension catalog
...
> v1.5.2 and will not change between rcs.
>
> Live build + release status across all Haybarn repos:
> **<https://haybarn-status.query.farm>**.

This first Haybarn release line is built from **DuckDB v1.5.2**.
...
# PyPI (no install)
uvx haybarn-cli==1.5.2rc13          # or `pipx run haybarn-cli==1.5.2rc13`
...
commit. Once `1.5.2` final lands, `npx haybarn` and `uvx haybarn-cli` will work
